...
def create_data_loader(input, context_length, vocab, device, batch_size = 32, shuffle = False):
    samples = input.split()
    i = 0
    context = []
    ...
    return torch.utils.data.DataLoader(torch.Tensor(context, device=device).type(torch.long) , batch_size=batch_size, shuffle=shuffle)


def data_prep_from_file(file_path, device , context_length = 4, train_p = 0.6, valid_p = 0.2):
    ...
    train_dl = create_data_loader(train_text, context_length, train_vocab , device)
    valid_dl = create_data_loader(valid_text, context_length, train_vocab,  device)
    test_dl = create_data_loader(test_text, context_length, train_vocab, device)
